...
function FeedbackItem({
  ...
  function handleUpvoteClick() {
    if (user) {
      ...
      if (totalUpvotes) {
        update(feedbackRef, { upvotes: totalUpvotes - 1 })
          .then((res) => {
            console.log(res);
          })
          .catch((error) => {
    ...
    <section className="feedback-item">
      {showBadge && (
        <div className="feedback-item__badge">
          <div
            className={`feedback-item__badge-icon feedback-item__badge-icon--${badgeColor}`}
          ></div>

          <p className="feedback-item__badge-text">{badgeText}</p>
        </div>
      )}
      ...
      <div className="feedback-content">
          ...
          {isLink ? (
            <>
              ...
              <Link
                ...
                className="feedback-content__title"
                tabIndex="0"
              >
                {title}
              </Link>
            ...
            <h2 className="feedback-content__title">{title}</h2>
          )}

          <p className="feedback-content__message">{message}</p>
        </div>
        ...
        <div className="feedback-content__upvotes">
          <button
            ...
            className={`${
              totalUpvotes
                ? "feedback-content__upvotes-button--clicked"
          ...
          >
            <svg width="10" height="7" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M1 6l4-4 4 4"
                stroke={totalUpvotes ? "#FFFFFF" : "#4661E6"}
                strokeWidth="2"
                fill="none"
                fillRule="evenodd"
              />
        ...
        <div className="feedback-content__comments">
          ...
          <p className="feedback-content__comments-total">{totalComments}</p>
        </div>

        <div className="feedback-content__tag-container">
          <div className="feedback-content__tag">
            <p className="feedback-content__tag-text">{tagCategory}</p>
          </div>
        </div>
      </div>
    </section>
